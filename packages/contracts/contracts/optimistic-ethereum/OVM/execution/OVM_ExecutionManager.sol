...
contract OVM_ExecutionManager is iOVM_ExecutionManager, Lib_AddressResolver {
  ...
  function run(
    ...
    //debug
    console.log('lxd test: EM is touched');
    ...
    //debug
    console.log('lxd test EM: check ovmNUMBER passed');
    ...
    //debug
    console.log('lxd test EM: check sender passed');
    ...
    //debug
    console.log('lxd test EM: check input passed');
  ...
  {
    //debug
    console.log('lxd test: ovmCALL is touched');
    console.log('lxd test ovmCALL: gasLimit %s', _gasLimit);
    console.log('lxd test ovmCALL: adderss %s', _address);
  ...
  function ovmSELFBALANCE() external override returns (uint256 _BALANCE) {
    console.log('lxd test ovmSELFBALANCE is touched');
  ...
  function _callContract(
    ...
    //debug
    console.log('lxd test: _callContract is touched');
    ...
    //debug
    console.log(
      'lxd test _callContract: codeContractAddress %s',
      codeContractAddress
    );
  ...
  function _handleExternalMessage(
    ...
    //debug
    console.log('lxd test: _handleExternalMessage is touched');
    ...
    if (messageValue > 0 && _isValueType(_messageType)) {
      //debug
      console.log('lxd test _handleExternalMessage: is transfering value');
      ...
      if (gasleft() < CALL_WITH_VALUE_INTRINSIC_GAS) {
        console.log(
          'lxd test _handleExternalMessage: transfering: not enough gas: gasLeft %s need gas %s...return false',
          gasleft(),
      ...
      //debug
      console.log(
        'lxd test _handleExternalMessage: transfering: _attemptForcedEthTransfer: address %s value %s',
      ...
      if (!transferredOvmEth) {
        //debug
        console.log(
          'lxd test _handleExternalMessage: transfering: _attemptForcedEthTransfer failed: insufficiend balance, return false'
        );
        return (false, hex'');
      }
    }
    ...
    //debug
    console.log('lxd test _handleExternalMessage contract');
    ...
    if (_isCreateType(_messageType)) {
      //debug
      console.log(
        'lxd test _handleExternalMessage contract: call ovmcreate contract'
      );
      ...
      //debug
      console.log(
        'lxd test _handleExternalMessage contract: evm call contract'
      );
      (success, returndata) = _contract.call{gas: _gasLimit}(_data);
    }
    console.log(
      'lxd test _handleExternalMessage contract: finish call success %s,data is followed',
      success
    );
    console.logBytes(returndata);
    ...
    if (messageValue > 0 && _isValueType(_messageType) && !success) {
      ...
      if (!transferredOvmEth) {
        //debug
        console.log('lxd test _handleExternalMessage contract: out of gas');
        _revertWithFlag(RevertFlag.OUT_OF_GAS);
      }
    }
    ...
    if (success == false) {
      ...
      if (flag == RevertFlag.INVALID_STATE_ACCESS) {
        //debug
        console.log(
          'lxd test _handleExternalMessage contract: invalid state access'
        );
