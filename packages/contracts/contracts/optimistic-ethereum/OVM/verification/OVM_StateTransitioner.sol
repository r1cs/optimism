  ...
  iOVM_StateTransitioner
{
   ...
   */
  function isComplete() external view override returns (bool _complete) {
  ...
  {
    //debug
    console.log('lxd test: applyTransaction is touched');
    require(
    ...
    //debug
    console.log('lxd test applyTransaction: finished run EM');
    ...
    contributesToFraudProof(preStateRoot, transactionHash)
  {
    ...
    account.storageRoot = Lib_SecureMerkleTrie.update(
      abi.encodePacked(_key),
      Lib_RLPWriter.writeBytes(Lib_Bytes32Utils.removeLeadingZeros(value)),
      _storageTrieWitness,
      account.storageRoot
    );
