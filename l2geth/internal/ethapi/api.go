...
func (s *PublicBlockChainAPI) GetStateDiffProof(ctx context.Context, blockNrOrHash rpc.BlockNumberOrHash) (*StateDiffProof, error) {
  ...
  var changedAccounts []AccountResult
  ...
  for address, keys := range diffs {
    ...
    //add changeed key
    changedKey := make([]string, 0, len(keys))
    ...
    //get changed key proof for post state
    nextStateNumber := rpc.BlockNumber(header.Number.Int64() + 1)
    changedProof, err := s.GetProof(ctx, address, changedKey, rpc.BlockNumberOrHash{BlockNumber: &nextStateNumber})
    if err != nil {
      return nil,err
    }

    accounts = append(accounts, *res)
    changedAccounts=append(changedAccounts,*changedProof)
  }
